...
{
	sprintf(M_buf1, "{%u}", static_cast<unsigned int>(m_sArgIndex++));
...
String& String::arg(const bool& v)
{
	sprintf(M_buf1, "%s", v ? "true":"false");
	sprintf(M_buf2, M_buf1, v);
	sprintf(M_buf1, "{%u}", static_cast<unsigned int>(m_sArgIndex++));
	replace(this, this, M_buf1, M_buf2, std::strlen(M_buf1), std::strlen(M_buf2));
	return *this;
...
String String::toGBK()const
{
    ...
    int i = MultiByteToWideChar(CP_UTF8, 0, m_pData, -1, NULL, 0);
    strSrc = new WCHAR[i + 1];
    MultiByteToWideChar(CP_UTF8, 0, m_pData, -1, strSrc, i);
...
int String::find(const char* src, const size_t& begin)const
{
...
int String::find(const String& src, const size_t& begin)const
{
	if (begin > m_sLen) return -1;
...
int String::rfind(const char* src)const
{
	size_t bgn = m_sLen;
	size_t count = std::strlen(src);
	if (bgn >= count)
...
int String::rfind(const String& src)const
{
	size_t bgn = m_sLen;
	size_t count = src.m_sLen;
	if (bgn >= count)
...
bool String::endWith(const char* str) const{ 
	...
	if(index == -1)
		return false;
	return m_sLen - index == std::strlen(str); 
}
bool String::endWith(const String& str) const{
	int index = find(str);
	if(index == -1)
		return false;
	return m_sLen - index == str.m_sLen; 
}
